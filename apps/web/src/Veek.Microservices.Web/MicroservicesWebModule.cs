...
using Veek.Microservices.AdministrationService.Web;
using Veek.Microservices.IdentityService;
using Veek.Microservices.IdentityService.Web;
using Veek.Microservices.Localization;
using Veek.Microservices.ProductService;
using Veek.Microservices.ProductService.Web;
using Veek.Microservices.SaasService;
using Veek.Microservices.SaasService.Web;
using Veek.Microservices.Shared.Hosting.AspNetCore;
using Veek.Microservices.Web.Navigation;
using Prometheus;
...
public class MicroservicesWebModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(MicroservicesWebModule).Assembly
            );
        });
    ...
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        {
            options.MenuContributors.Add(new MicroservicesMenuContributor(configuration));
        });

        Configure<AbpToolbarOptions>(options =>
        {
            options.Contributors.Add(new MicroservicesToolbarContributor());
        });
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<ProductServiceWebModule>(Path.Combine(
                    ...
                    $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}services{Path.DirectorySeparatorChar}product{Path.DirectorySeparatorChar}src{Path.DirectorySeparatorChar}Veek.Microservices.ProductService.Web"));
            });
        }
    }
